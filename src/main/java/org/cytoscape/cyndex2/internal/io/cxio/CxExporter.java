...
import org.ndexbio.model.cx.SubNetDataElement;
...
public final class CxExporter {
	...
	public final boolean writeNetwork(final CyNetwork network, final boolean write_siblings, final AspectSet aspects,
		...
		if (!write_siblings) {
			...
			if (table.getColumn(StringResources.UUID_COLUMN) != null) {
				hasNDExData = true;
			}
		}
		...
		MetaDataCollection pre_meta_data = addPreMetadata(network, write_siblings, w, 1L, hasNDExData,
				!groupNodeIds.isEmpty());
		...
		if (hasNDExData && CXInfoHolder.getNamespaces(network) != null
				&& CXInfoHolder.getNamespaces(network).size() > 0) {
			final List<AspectElement> nsAspect = new ArrayList<>(1);
			nsAspect.add(CXInfoHolder.getNamespaces(network));
		...
		ProvenanceEntity oldProvenanceEntity = ((hasNDExData && CXInfoHolder.getProvenance(network) != null)
				? CXInfoHolder.getProvenance(network).getEntity()
				: null);
		...
		try {
			...
			if (hasNDExData) {
				for (Map.Entry<String, Collection<AspectElement>> entry : CXInfoHolder.getOpaqueAspectsTable(network)
						.entrySet()) {
		...
		if (success) {
			...
			CXInfoHolder.setMetadata(network, pre_meta_data);
	...
	public static Long getNodeIdToExport(CyNode cyNode, CyNetwork network) {
		...
		MetaDataCollection metadata = CXInfoHolder.getMetadata(network);
		if (metadata == null) {
			return id;
		}
		Long counter = metadata.getIdCounter(NodesElement.ASPECT_NAME);
		if (counter != null) {
			Long cxNodeId = CXInfoHolder.getCXNodeId(network, id);
			...
			CXInfoHolder.addNodeMapping(network, id, newid);
			metadata.setIdCounter(NodesElement.ASPECT_NAME, newid);
			try {
				CXInfoHolder.setMetadata(network, metadata);
	...
	public static long getEdgeIdToExport(CyEdge cyedge, CyNetwork network) {
		...
		if (counter != null) {
			Long cxEdgeId = CXInfoHolder.getCXEdgeId(network, Long.valueOf(id));
			...
			CXInfoHolder.addEdgeMapping(network, Long.valueOf(id), Long.valueOf(newid));
	...
	private final static MetaDataCollection addPostMetadata(final CxWriter w, final AspectElementCounts aspects_counts,
		...
		if (CXInfoHolder.hasUUID(network)) {
			addDataToPostMetaDataCollection(post_meta_data, NodesElement.ASPECT_NAME, null,
					CXInfoHolder.getMetadata(network).getIdCounter(NodesElement.ASPECT_NAME));
			addDataToPostMetaDataCollection(post_meta_data, EdgesElement.ASPECT_NAME, null,
					CXInfoHolder.getMetadata(network).getIdCounter(EdgesElement.ASPECT_NAME));
	...
	private final static MetaDataCollection addPreMetadata(final CyNetwork network, final boolean write_siblings,
			final CxWriter w, final Long consistency_group, boolean hasNDExData, boolean hasGroup) {
	...
	private final static boolean isIgnore(final String column_name, final Set<String> additional_to_ignore,
		...
		if (setttings.isIgnoreSuidColumn() && column_name.equals(CxUtil.SUID)) {
			...
			return true;
		} else if (column_name.equals(StringResources.SERVER_URL_COLUMN)
				|| column_name.equals(StringResources.UUID_COLUMN) || column_name.equals(StringResources.CX_ID_COLUMN)
				|| column_name.equals(StringResources.METADATA_COLUMN)
				|| column_name.equals(StringResources.NAMESPACES_COLUMN)
				|| column_name.equals(StringResources.PROVENANCE_COLUMN)
				|| column_name.startsWith(StringResources.OPAQUE_ASPECTS_COLUMN_PREFIX)) {
			return true;
		}
		return false;
	}
	...
	private final static void writeNodeAttributes(final CyNetwork network, final boolean write_siblings,
			final CxWriter w, final String namespace, Set<Long> groupNodeIds) throws IOException {
	...
	private final void writeSubNetworks(final CyNetwork network, final boolean write_siblings, final CxWriter w,
		...
		if (write_siblings) {
			...
			writeSubNetworkNDExData(my_root, w);
	...
	private void writeSubNetworkNDExData(CyRootNetwork root, CxWriter w) throws IOException {
		List<AspectElement> subNetData = new ArrayList<AspectElement>();

		for (CySubNetwork network : root.getSubNetworkList()) {
			Long suid = network.getSUID();
			SubNetDataElement subNet = new SubNetDataElement(suid);

			subNet.setMetadata(CXInfoHolder.getMetadata(network));
			subNet.setNamespace(CXInfoHolder.getNamespaces(network));
			subNet.setProvenance(CXInfoHolder.getProvenance(network));
			subNet.setOpaqueAspectTable(CXInfoHolder.getOpaqueAspectsTable(network));
			subNetData.add(subNet);
		}

		w.writeAspectElements(subNetData);
